...
class CrossValidation:
    ...
    def split(self):
        ...
        if unique_values == 1:
        ...
        elif self.problem_type in ('single_col_regession', 'multi_col_regression'):
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X=self.dataframe)):
                self.datafarame.loc[val_idx, 'kfold'] = fold

        elif self.problem_type.startswith('holdout_'):
            holdout_percentage = int(self.problem_type.split('_'))[1]
            num_holdout_samples = int(self.dataframe*holdout_percentage/100)
...
if __name__ == '__main__':
    ...
    cv = CrosssValidation(df, shuffle=True, target_cols=['attribute_ids'], 
                        problem_type='multiple_classification', 
                        multilabel_delimiter=' ')
    df_split = cv.split()
    print(df_split.head())
